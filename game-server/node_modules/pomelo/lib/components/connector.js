...
pro.send = function(reqId, route, msg, recvs, opts, cb) {
  if(__G.DEBUG_NET_MSG)logger.debug('[%s] send message reqId: %s, route: %s, msg: %j, receivers: %j, opts: %j', this.app.serverId, reqId, route, msg, recvs, opts);
...
var getSession = function(self, socket) {
  ...
  session = self.session.create(sid, app.getServerId(), socket);
    if(__G.DEBUG_NET_MSG)logger.debug('[%s] getSession session is created with session id: %s', app.getServerId(), sid);
  ...
  session.on('bind', function(uid) {
      if(__G.DEBUG_NET_MSG)logger.debug('session on [%s] bind with uid: %s', self.app.serverId, uid);
...
var handleMessage = function(self, session, msg) {
    if(__G.DEBUG_NET_MSG)logger.debug('[%s] handleMessage session id: %s, msg: %j', self.app.serverId, session.id, msg);
